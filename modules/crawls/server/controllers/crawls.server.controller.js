...
exports.create = function (req, res) {
  ...
  crawl.save(function (err) {
    if (err) {
      logger.error('Crawl cannot be created.', {error:err, user:crawl.user.toObject()});
...
exports.update = function (req, res) {
  ...
  crawl.save(function (err) {
    if (err) {
      logger.error('Crawl cannot be updated.', {error:err, user:crawl.user.toObject()});
      return res.status(400).send({
        message: errorHandler.getErrorMessage(err)
      });
    } else {
    ...
    if (err) {
      ...
      crawl.crawler.update(
        ...
        function (err, rawResponse) {
          if (err) {
            logger.error('Crawl\'s Crawler cannot be updated.', {error:err, rawResponse:rawResponse, user:crawl.user.toObject()});
          } else {
            logger.info('Target crawl record have been deleted.', {crawlId: crawl._id.toString(), crawlerId: crawl.crawler._id.toString(), user:crawl.user.toObject()});
            res.json(crawl);
          }
        }
      );
...
exports.list = function (req, res) {
  Crawl.find().sort('-created').populate('user', 'displayName').exec(function (err, crawls) {
    if (err) {
      logger.error('Crawl cannot be listed.', {error:err});
      return res.status(400).send({
        message: errorHandler.getErrorMessage(err)
      });
    } else {
...
exports.crawl = function (job, done) {
  Crawler.findById(job.attrs.data.crawler_id).exec(function (err, pcrawler) {
    if (err) {
      ...
      logger.error('[crawler] Cannot be found.', {crawlerId: job.attrs.data.crawler_id, crawlerName: job.attrs.data.crawlerName, user: job.attrs.data.user, error:err});
    } else if (!pcrawler) {
      done(err);
      logger.error('[crawler] No crawler with that identifier has been found.', {crawlerId: job.attrs.data.crawler_id, crawlerName: job.attrs.data.crawlerName, userDisplayName: job.attrs.data.userDisplayName});
    }
    ...
    pcrawler.save(function (err) {
      if (err) {
        ...
        logger.error('[crawler] Cannot be saved', {crawlerId: job.attrs.data.crawler_id, crawlerName: job.attrs.data.crawlerName, userDisplayName: job.attrs.data.userDisplayName, error:err});
      }
