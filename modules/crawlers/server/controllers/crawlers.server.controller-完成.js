...
exports.update = function (req, res) {
  ...
  exports.requestUrlList([{crawler: crawler}, function (urls, err) {
...
exports.requestUrlList = function (args) {
  ...
  if (args[0].crawler.strategy.selected === 'api' && args[0].crawler.strategy.content.api.length >= 1) {
    Crawler.find({"_id": {$in: args[0].crawler.strategy.content.api}}).populate('crawls', 'record').exec(function (err, apicrawler) {
      if (err) {
        logger.error('[crawler] Cannot be found.', {crawlerId: args[0].crawler.strategy.content.api, error:err});
      } else if (!apicrawler) {
        logger.error('[crawler] No crawler with that identifier has been found.', {crawlerId: args[0].crawler.strategy.content.api});
      }
...
agenda.define('Crawler Crawl', {concurrency: 1}, function ( job, done ) {
  var a = require(path.resolve('./modules/crawls/server/controllers/crawls.server.controller.js'));
  a.crawl(job,done);
